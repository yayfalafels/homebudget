...
def update_income(
    ...
    if amount is None and foreign_amount is None and (currency is not None or rate is not None):
        raise click.UsageError(
            "Income update: Provide --amount or --currency-amount when setting currency fields."
        )
    if amount is not None and foreign_amount is not None:
        raise click.UsageError("Income update: Provide --amount or --currency-amount, not both.")
    if foreign_amount is not None and (currency is None or not currency.strip()):
        raise click.UsageError("Income update: --currency is required with --currency-amount.")
    with get_client(ctx) as client:
