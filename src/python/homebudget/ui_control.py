...
class HomeBudgetUIController:
    ...
    def open(
        ...
        try:
            ...
            if not verify:
                print("[OK] Open command executed (verification disabled)")
            ...
            for attempt in range(1, verify_attempts + 1):
                if HomeBudgetUIController._window_exists():
                    print(f"[OK] Application window detected (attempt {attempt}/{verify_attempts})")
                    
                    # Allow application to fully initialize after window appears
                    print(f"  Allowing {settle_time}s for application to settle...")
                    time.sleep(settle_time)
                    
                    print(f"[OK] Application fully initialized and ready")
            ...
            print(f"[WARN] Could not verify window after {verify_attempts} attempts")
            return (
                False,
    ...
    def close(
        ...
        try:
            ...
            if hwnd == 0:
                ...
                print("[WARN] Window not found (may be already closed)")
            ...
            if process_id == 0:
                print("[WARN] Could not determine process ID")
            ...
            if result.returncode == 0:
                print(f"[OK] taskkill /PID {process_id} succeeded")
            elif result.returncode == 128:
                print(f"[WARN] Process {process_id} not found (may have closed)")
            else:
                print(f"[WARN] taskkill return code: {result.returncode}")
            
            if not verify:
                print("[OK] Close command executed (verification disabled)")
    ...
    def apply_changes_with_ui_control(
        ...
        if not close_success and close_verify:
            print(f"[ERROR] Failed to close UI: {close_msg}")
